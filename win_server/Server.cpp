...
int Server::init()
{
    ...
    if(0 != bind(host_socket_, (struct sockaddr*) &host_addr_, sizeof(host_addr_)))
    {
...
int Server::listen_thread_proc()
{
    ...
    while(!clear_up_)
    {
		int clnt_addr_size_ = sizeof(client_addr_);
...
int Server::work_thread_proc()
{
    while( -1 != clnt_socket_ )
    {
        ...
        int len = recv(clnt_socket_,(char*)_recv_buf, _recv_buf_len, 0);
        ...
        std::cout << "msg value:" << _recv_buf << std::endl;
		// TODO send to ddr
